...
\begin{center}
    \begin{tikzpicture}[x=0.9cm, y=1cm]
        ...
        \draw[very thick, black, domain=-pi:0, samples=100] plot (\x, {cos(\x r)});
        
        % Complétion par périodicité sur [pi, 2pi] et [-2pi, -pi]
        \draw[very thick, black, domain=pi:2*pi, samples=100] plot (\x, {cos(\x r)});
        \draw[very thick, black, domain=-2*pi:-pi, samples=100] plot (\x, {cos(\x r)});
        
        % Ajout du nom de la courbe (C_f)
        \node[above] at (2*pi, 1) {$(C_f)$};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[x=0.9cm, y=1cm]
        ...
        \draw[very thick, black, domain=-pi:0, samples=100] plot (\x, {sin(\x r)});
        
        % Complétion par périodicité sur [pi, 2pi] et [-2pi, -pi]
        \draw[very thick, black, domain=pi:2*pi, samples=100] plot (\x, {sin(\x r)});
        \draw[very thick, black, domain=-2*pi:-pi, samples=100] plot (\x, {sin(\x r)});
        
        % Ajout du nom de la courbe (C_g)
        \node[above] at (pi/2, 1) {$(C_g)$};
    \end{tikzpicture}
\end{center}
...
\begin{enumerate}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1]
            ...
            \draw[step=1cm,gray,very thin,dashed] (-3,-1) grid (3,5);
            \foreach \x in {-2,-1,1,2} \draw (\x,0.1) -- (\x,-0.1) node[below] {$\x$};
            \foreach \y in {1,2,3,4} \draw (0.1,\y) -- (-0.1,\y) node[left] {$\y$};
            \node[below left] at (0,0) {$O$};
            
            \draw[very thick, blue, domain=-2.2:2.2, smooth, variable=\x] plot ({\x}, {\x*\x}) node[right] {$\big(\mathcal{C}_f\big)$};
